        ...
        <>
          <div className="grid gap-3 xl:grid-cols-2">
            ...
            <article className="rounded-2xl border border-slate-800 bg-slate-950/50 p-3" role="img" aria-label="NDVI and land surface temperature relationship chart">
              <h3 className="mb-1 text-sm font-medium text-slate-200">NDVI vs LST relationship</h3><p className="mb-2 text-[10px] text-slate-500">Association in the selected dataset; no causal conclusion is implied.</p>
          ...
          <article className="mt-3 rounded-2xl border border-slate-800 bg-slate-950/50 p-3" role="img" aria-label="Land-cover distribution chart">
            <div className="mb-3"><h3 className="text-sm font-medium text-slate-200">Land-cover distribution</h3><p className="mt-1 text-[10px] text-slate-500">Categorized surface types supplied as contextual data.</p></div>
            {landCover.length === 0 ? <EmptyChart message="Land-cover data is unavailable for this dataset." /> : <div className="grid gap-3 lg:grid-cols-[1.2fr_0.8fr]"><div className="h-52"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={landCover} dataKey="percentage" nameKey="label" innerRadius={42} outerRadius={70} paddingAngle={3}>{landCover.map((entry) => <Cell key={entry.categoryId ?? entry.label} fill={entry.color} />)}</Pie><Tooltip formatter={(value) => [`${Number(value ?? 0)}%`, 'Share']} contentStyle={{ background: '#0f172a', borderColor: '#334155', borderRadius: 12 }} /><Legend /></PieChart></ResponsiveContainer></div><div className="space-y-2">{landCover.map((entry) => <div key={entry.categoryId ?? entry.label} className="flex items-center justify-between gap-3 rounded-xl border border-slate-800 bg-slate-900/80 px-2.5 py-1.5"><div className="flex min-w-0 items-center gap-2"><span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: entry.color }} /><span className="truncate text-xs text-slate-200">{entry.label}</span></div><span className="text-xs font-medium text-slate-50">{entry.percentage}%</span></div>)}</div></div>}
          </article>
        </>
      )}
    </section>
